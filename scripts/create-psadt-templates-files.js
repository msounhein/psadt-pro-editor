 ...
 * and saves template content as files.
 */
...
// Path where template files will be stored
...
async function createPsadtTemplates() {
  ...
  try {
    ...
    if (!existingTemplateNames.includes('PSAppDeployToolkit_Documentation_v3')) {
      const templateName = 'PSAppDeployToolkit_Documentation_v3';
      const v3Content = createV3Documentation();
      const storagePath = saveTemplateContent(templateName, v3Content);
    ...
    if (!existingTemplateNames.includes('PSAppDeployToolkit_Documentation_v4')) {
      const templateName = 'PSAppDeployToolkit_Documentation_v4';
      const v4Content = createV4Documentation();
      const storagePath = saveTemplateContent(templateName, v4Content);
    ...
    if (!existingTemplateNames.includes('PSAppDeployToolkit_Migration_Guide')) {
      const templateName = 'PSAppDeployToolkit_Migration_Guide';
      const migrationContent = createMigrationGuide();
      const storagePath = saveTemplateContent(templateName, migrationContent);
    ...
    if (!existingTemplateNames.includes('PSAppDeployToolkit_Base_Template')) {
      const templateName = 'PSAppDeployToolkit_Base_Template';
      const baseContent = createBaseTemplate();
      const storagePath = saveTemplateContent(templateName, baseContent);
    ...
    if (templatePromises.length > 0) {
      ...
      for (const template of results) {
        console.log(`- ${template.name} (${template.storagePath})`);
      }
    } else {
 ...
 * @returns {string} - Path where the content was saved
 */
function saveTemplateContent(templateName, content) {
  // Create a safe filename
  const safeFileName = templateName.replace(/[^a-z0-9]/gi, '_').toLowerCase() + '.md';
  const filePath = path.join(templatesDir, safeFileName);
  
  // Write content to file
  fs.writeFileSync(filePath, content, 'utf8');
  console.log(`Saved template content to ${filePath}`);
