...
export default function DocumentationPage() {
  ...
  // Use lazy initial loading with useEffect and a flag
  ...
  }, [initialLoadComplete, fetchDocumentationList]);

  // Modified useEffect to show commands without unnecessary animations
  useEffect(() => {
    if (Object.keys(commandGroups).length > 0) {
      ...
      if (firstGroupWithCommands) {
        ...
        if (allCommands.length > 0) {
          // Don't animate all commands, just set the first one as selected
  ...
  const fetchDocumentationList = useCallback(async () => {
    setIsLoading(true);
    try {
      ...
      console.log('Fetching documentation with URL:', url);
  ...
  }, [selectedPrefix, searchTerm, toast]);

  const fetchDocumentationFile = useCallback(async (filePath: string) => {
    try {
  ...
  const getFileIcon = (fileName: string) => {
    ...
    if (extension === 'md') {
      // Memoize the entire return to prevent unnecessary re-renders
    ...
    try {
      ...
      if (sessionStatus === 'unauthenticated' || !session) {
        toast({
          ...
          description: 'You must be logged in to refresh documentation.',
          variant: 'destructive'
        });
        setIsRefreshing(false);
        return;
      ...
      // Call the API endpoint to trigger the PowerShell script for v4 only
      const response = await fetch('/api/docs/refresh', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // Include the auth token if available via session
  ...
  // Render the command filter bar with memoization
  const renderCommandFilterBar = useMemo(() => {
  ...
  }, [commandPrefixes, commandGroups, expandedGroups, selectedPrefix, selectedCommand, lastClickedCommand, vscodePalette, fileTree]);

  // Function to render the file tree with memoization
  ...
  }, [searchTerm, selectedPrefix, fileTree, activeDirectory, currentFilePath, vscodePalette]);
  ...
  ), [fileTree, searchTerm, selectedPrefix, activeDirectory, currentFilePath, vscodePalette, activeTab, isLoading, fileContent, mdxSource, fileType, commandPrefixes]);
