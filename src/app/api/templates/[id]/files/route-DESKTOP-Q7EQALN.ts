...
import { ensureTemplateDirectories, ensureDirectory } from '@/lib/filesystem/ensure-template-directories';
...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      // We need to handle different template path formats
      // For paths with userId/type/templateName structure, we can use ensureTemplateDirectories
      
      let directoryExists = false;
      const idParts = id.split('/');
      
      if (idParts.length > 1) {
        // This looks like a path with userId/type/templateName
        const userId = idParts[0];
        try {
          // Create directories for the user
          const templateDirInfo = await ensureTemplateDirectories(userId);
          console.log(`[FILES API] Created template directories for user:`, templateDirInfo);
          directoryExists = true;
        } catch (dirError) {
          console.error(`[FILES API] Error creating template directories for user ${userId}:`, dirError);
        }
      } else {
        // Try direct method
        console.log(`[FILES API] Using direct method to create directory`);
        const storageDir = process.env.STORAGE_DIR || 'storage';
        const templateDir = path.join(process.cwd(), storageDir, 'templates', id);
        directoryExists = await ensureDirectory(templateDir);
      }
      ...
      let templatePath = id;  // Default to the ID
      
      if (template) {
        // Use the storagePath from the database if available
        ...
        console.log(`[FILES API] Template not found in database with ID: ${id}, using ID as path`);
      }
      
      console.log(`[FILES API] Using template path: ${templatePath}`);
